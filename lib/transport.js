...
class SlsTransport extends Transport {
  constructor(options) {
    ...
    assert(this.sls.topic, 'should pass config.sls.topic');
  ...
  _format(level, args, meta) {
    ...
    if (meta && meta.ctx) {
      ...
      contents = contents.concat([
        {
          key: 'env',
          value: (ctx.app && ctx.app.env) || process.env.NODE_ENV
        },
        {
          key: 'method',
          value: (ctx.request && ctx.request.method) || '-'
        },
        {
          key: 'url',
          value: (ctx.request && ctx.request.url) || '-'
        },
        {
          key: 'traceId',
          value: (ctx.tracer && ctx.tracer.traceId && ctx.tracer.traceId.toString()) || '-'
        },
        {
          key: 'userId',
        ...
        },
        {
          key: 'x-session-id',
          value:
            (ctx.header && ctx.header['x-session-id'] && ctx.header['x-session-id'].toString()) ||
            '-'
        },
        {
          key: 'cookie',
          value: (ctx.header && ctx.header.cookie) || '-'
        },
        {
          key: 'request',
          value: (ctx.request && ctx.request.body && JSON.stringify(ctx.request.body)) || '-'
        },
        {
          key: 'response',
